...
class FleetTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        Bike.objects.create(id='BK_001', fleet=fleet1, status='unlocked',
                            latitude=self.AMSTERDAM_LAT, longitude=self.AMSTERDAM_LONG)
        Bike.objects.create(id='BK_002', fleet=fleet1, status='locked',
                            latitude=self.AMSTERDAM_LAT, longitude=self.AMSTERDAM_LONG)
        fleet2 = Fleet.objects.create(id='FL_002', name='Fleet two')
        Bike.objects.create(id='BK_003', fleet=fleet2, status='unlocked',
                            latitude=self.AMSTERDAM_LAT, longitude=self.AMSTERDAM_LONG)
    ...
    def test_create_fleet(self):
        ...
        self.assertEqual(response.status_code,
                         status.HTTP_201_CREATED, 'Returns CREATED')
    ...
    def test_delete_a_bike(self):
        ...
        self.assertEqual(response.status_code,
                         status.HTTP_200_OK, 'Returns OK')
        self.assertEqual(Fleet.objects.filter(
            id=bid).first(), None, 'Bike is gone')
        # Return deleted data in response
        data = JSONParser().parse(io.BytesIO(response.content))
        self.assertEqual(data['id'], bid)
